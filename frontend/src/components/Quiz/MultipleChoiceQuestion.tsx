...
import { CiCircleMinus, CiSquareChevUp, CiSquareChevDown, CiTrash, CiCirclePlus } from "react-icons/ci";
        ...
        <div className="flex flex-col w-full gap-y-3">
            ...
            <div className="flex flex-col w-full gap-y-2">
                <div className="flex flex-wrap gap-2">
                    ...
                    {availableUnits.map(unit => (
                        <div className="p-2 bg-white text-primary border-1 border-primary rounded-md">
                            {unit.name}
            ...
            <div className="flex flex-col w-full">
                ...
                <input 
                    ...
                    className="p-1 w-full text-black bg-green-200 border-1 border-black text-2xl outline-none" 
            ...
            <div className="flex flex-col w-full">
                ...
                <div className="flex flex-col w-full gap-y-1">
                    {alternateChoices.map((choice, i) => (
                        <div className="flex flex-row items-center w-full gap-x-2 bg-red-200 text-black border-1 border-black p-1" key={i}>
                            <input id={`alternate-choice-${i}`} className="flex flex-1 text-2xl outline-none" value={choice} onChange={(e) => {
                                const newChoices = [...alternateChoices]
                                newChoices[i] = e.target.value
                                setAlternateChoices(newChoices)
                            }}/>
                            { i === alternateChoices.length -1 && <IoIosAdd className="text-3xl text-black icon-responsive" onClick={() => {
                                setAlternateChoices(prev => [...prev, ""])
                            }}/> }
