...
from .models import Quiz, Question, MultipleChoiceQuestion, AnswerChoice
...
class MultipleChoiceQuestion(serializers.ModelSerializer):
    alternate_choices = AnswerChoiceSerializer(many=True)
    class Meta:
        model = Question
        fields = ['id', 'question_text', 'related_units', 'related_subunits', 'correct_answer', 'alternate_choices']
        read_only_fields = ['id']
